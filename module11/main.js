...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function printYearsSchooling(date) {
        ...
        return isFinished ? `${startYear}-${endYear} (закончил обучение)` : `${startYear}-${endYear} (${currentYear - startYear + 1} курс)`;
    }
    ...
    function createStudent(studentForm) {    
        ...
        saveStudent(student).then(savedStudent => {
        ...
        fetch('http://localhost:3000/students')
            ...
            .then(data => {
                ...
                studentsArray.forEach((student, index) => {
                    tableBody.appendChild(createStudentRow(student, index));
                });            
            })
    ...
    .then(response => response.json())
    .catch(error => console.error('Ошибка при сохранении студента:', error));
}

    function deleteStudent({ student, element }) {
        ...
        .then(() => {
            ...
            const index = studentsArray.findIndex(s => s.id === student.id);
            if (index > -1) {
    ...
    function createStudentRow(student) {
        ...
        birthDateCell.textContent = formatDateString(student.dateOfBirth);

        const yearsStudyCell = document.createElement('td');
        yearsStudyCell.textContent = printYearsSchooling(student.yearsStudy);

        const deleteButton = document.createElement('button');
        deleteButton.textContent = 'Удалить';
        deleteButton.classList.add('btn', 'btn-danger');
        deleteButton.addEventListener('click', () => {
            deleteStudent({ student, element: row });
        });
